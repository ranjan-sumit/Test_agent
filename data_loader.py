...
Files expected in: data/ subfolder next to app.py
"""
...
DATA_FILES = {
    "sales":    "data/Sales_HistoricalData_Structured.xlsx",
    "inv_lt":   "data/Inventory_Extract_and_Lead_Time.xlsx",
    "bom":      "data/Fi11_BOM_MResult_v2.xlsx",
    "mat_master": "data/Material_master_data_with_planning_parameters__Turku___Boston_.xlsx",
    "curr_inv": "data/Current_Inventory___planning_parameters__Turku_and_Boston_.xlsx",
}
...
def build_material_summary(data: dict) -> pd.DataFrame:
    ...
    for mat in df_lt["Material"].unique():
        ...
        # ── Data quality flags (surfaced in UI Data Quality tab) ──────────
        dq_flags = []
        if ss == 0:
            dq_flags.append("Safety stock = 0 in both sources — no threshold defined")
        if effective_lt == 1 and lead_time == 0 and inhouse_time == 0:
            dq_flags.append("Lead time = 0 in Material Master — using 1d fallback (data gap)")
        if lot_size == 0:
            dq_flags.append("Fixed lot size = 0 — cannot calculate minimum replenishment quantity")
        if len(nonzero_dem) < 6:
            dq_flags.append(f"Only {len(nonzero_dem)} months of non-zero demand (minimum 6 for reliable SS calculation)")
        if zero_periods > 15:
            dq_flags.append(f"Zero stock in {zero_periods}/{total_periods} inventory periods — possible make-to-order pattern")

        # ── Replenishment recommendation (stakeholder formula) ──────────────
        gap_to_ss = max(0.0, ss - current_stock)
        if ss > 0 and current_stock < ss:
            repl_qty = int(max(gap_to_ss, lot_size)) if lot_size > 0 else int(gap_to_ss)
            repl_triggered = True
            repl_reason = f"max(gap_to_SS={int(gap_to_ss)}, lot_size={int(lot_size)}) = {repl_qty} units"
        else:
            repl_qty = 0
            repl_triggered = False
            repl_reason = "Stock above safety stock" if ss > 0 else "No safety stock configured"
        ...
        rows.append({
            ...
            "repl_triggered":        repl_triggered,
            "repl_quantity":         repl_qty,
...
def get_material_context(data: dict, material: str, summary_df: pd.DataFrame) -> dict:
    ...
    # Compute replenishment recommendation inline
    ss_ctx      = row["safety_stock"]
    lot_ctx     = row["lot_size"]
    stock_ctx   = row["current_stock"]
    gap_ctx     = max(0.0, ss_ctx - stock_ctx)
    repl_ctx    = {}
    if ss_ctx > 0 and stock_ctx < ss_ctx:
        qty_ctx = int(max(gap_ctx, lot_ctx)) if lot_ctx > 0 else int(gap_ctx)
        repl_ctx = {
            "trigger":   True,
            "quantity":  qty_ctx,
            "gap_to_ss": int(gap_ctx),
            "reason":    f"max(gap_to_SS={int(gap_ctx)}, lot_size={int(lot_ctx)}) = {qty_ctx} units",
            "data_gap":  lot_ctx == 0,
        }
    else:
        repl_ctx = {
            ...
            "quantity":  0,
            "gap_to_ss": 0,
            "reason":    "Stock above safety stock" if ss_ctx > 0 else "No safety stock configured",
            "data_gap":  ss_ctx == 0,
        }

    # BOM supplier analysis
    missing_suppliers = bom[bom["Supplier Name(Vendor)"].isna()]
    named_suppliers   = bom[bom["Supplier Name(Vendor)"].notna()]["Supplier Name(Vendor)"].unique().tolist()
    ...
    return {
        ...
        "spike_events":         spikes,
        "stock_recent_6m":      stock_hist.tail(6)[["Fiscal Period", "Gross Stock"]].to_dict("records"),
        "bom_components":       bom_summary,
        "total_bom_components": len(bom_summary),
        "replenishment_recommendation": repl_ctx,
        "missing_supplier_count": int(len(missing_suppliers)),
        "named_suppliers":        named_suppliers[:6],
        "data_quality_flags":     row.get("data_quality_flags", []),
        "parameter_sources": {
            "safety_stock": "Material Master (Current Inventory = 0 for all SKUs — data quality issue)",
            "lead_time":    "Material Master max(Planned Delivery, Inhouse Production)",
            "lot_size":     "Material Master Fixed Lot Size",
            "demand":       "Sales file — includes write-offs and internal consumption",
        },
    }
